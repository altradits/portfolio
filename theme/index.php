...
<main id="primary" class="site-main flex-1 w-full">
    ...
    <!-- HERO SECTION (Light Luminous Canvas with Story Slideshow) -->
    <section class="relative min-h-screen w-full bg-[#FAFAFA] flex flex-col items-center justify-center px-4 sm:px-8 lg:px-12 overflow-hidden">
        ...
        <!-- Light Theme Luminous Scrim Overlays -->
        <div class="absolute inset-0 z-1 bg-gradient-to-b from-white/92 via-white/80 to-[#FAFAFA] pointer-events-none"></div>
        ...
        <div class="absolute inset-0 z-1 bg-[radial-gradient(circle_at_center,rgba(229,82,82,0.08)_0%,transparent_60%)] pointer-events-none"></div>

        <!-- Interactive Luminous Particle Effect Canvas -->
        <canvas id="hero-particle-canvas" class="absolute inset-0 z-2 pointer-events-none w-full h-full"></canvas>
        ...
        <div class="absolute inset-0 z-1 flex items-center justify-center pointer-events-none select-none overflow-hidden">
            <img src="<?php echo function_exists('get_template_directory_uri') ? get_template_directory_uri() . '/assets/hero-watermark.svg' : 'assets/hero-watermark.svg'; ?>" 
                 alt="" 
                 class="w-[1400px] max-w-[150vw] opacity-[0.06] blur-[0.5px] transform -rotate-3 scale-110" />
        </div>
        ...
        <div class="relative z-10 w-full max-w-5xl 2xl:max-w-6xl mx-auto text-center flex flex-col items-center justify-center mt-24 sm:mt-20">
            <h1 class="text-4xl sm:text-6xl md:text-7xl lg:text-8xl font-extrabold tracking-tight text-zinc-950 mb-6 leading-tight drop-shadow-sm">
                Build Fast Software. Settle Payments Anywhere.
            </h1>
            
            <p class="text-lg sm:text-xl lg:text-2xl text-zinc-700 max-w-3xl mx-auto mb-10 leading-relaxed font-normal">
            ...
            <div class="flex flex-wrap items-center justify-center gap-4 w-full">
                ...
                <a href="#code" class="px-8 sm:px-10 py-3.5 sm:py-4 rounded-full bg-white hover:bg-zinc-100 border border-zinc-300 text-zinc-900 font-semibold text-sm sm:text-base transition-all hover:scale-105 active:scale-95 shadow-md">
                    View My Code
                </a>
            </div>
        </div>
...
<script>
    ...
    // Interactive Luminous Particle & Velocity Constellation Engine
    class ParticleNetwork {
        constructor(canvasId) {
            ...
            this.colors = [
                { r: 74, g: 111, b: 195 },  // Altradits Blue #4A6FC3
                { r: 229, g: 82, b: 82 },   // Coral-Red #E55252
                { r: 217, g: 119, b: 6 },   // Bitcoin Gold #D97706
                { r: 5, g: 150, b: 105 }    // Emerald Velocity #059669
            ];
        ...
        createParticles() {
            this.particles = [];
            const count = Math.floor(Math.min(95, Math.max(35, (this.width * this.height) / 13500)));
        ...
        animate() {
            ...
            for (let i = 0; i < this.particles.length; i++) {
                ...
                const currentR = p.radius + Math.sin(p.pulseVal) * 0.6;
                ...
                if (this.mouse.x !== null && this.mouse.y !== null) {
                    ...
                    if (dist < this.mouse.radius) {
                        ...
                        this.ctx.strokeStyle = `rgba(${p.color.r}, ${p.color.g}, ${p.color.b}, ${rayAlpha})`;
                ...
                // Halo glow
                const gradient = this.ctx.createRadialGradient(p.x, p.y, 0, p.x, p.y, currentR * 3);
                ...
                this.ctx.beginPath();
                this.ctx.arc(p.x, p.y, currentR * 3, 0, Math.PI * 2);
                ...
                this.ctx.fillStyle = `rgba(${p.color.r}, ${p.color.g}, ${p.color.b}, ${p.alpha * 1.2})`;
                this.ctx.fill();
            }
            ...
            for (let i = 0; i < this.particles.length; i++) {
                for (let j = i + 1; j < this.particles.length; j++) {
                    ...
                    if (dist < maxDist) {
                        ...
                        this.ctx.strokeStyle = `rgba(${p1.color.r}, ${p1.color.g}, ${p1.color.b}, ${lineAlpha})`;
    ...
    document.addEventListener('DOMContentLoaded', () => {
        new ParticleNetwork('hero-particle-canvas');
    });
